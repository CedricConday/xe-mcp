...
export const pairSummaryTool = {
  name: "pair_summary",
  description:
    "One-call morning briefing for a currency pair: current rate, 30-day range, annualised volatility, send-window verdict, and SMA(20). Combines get_rate + volatility_analysis + optimal_send_window into a single tool.",
  inputSchema: {
    type: "object",
    properties: {
      from: { type: "string", description: "Base currency (e.g. NZD)" },
...
export async function handlePairSummary(args: { from: string; to: string }): Promise<string> {
  const n = 30;
  const rates = await fetchHistoricalSeries(args.from, args.to, n);
  ...
  const lines = [
    ...
    `30d range:     ${sorted[0].toFixed(6)} – ${sorted[sorted.length - 1].toFixed(6)}`,
    `30d percentile: ${percentile}th`,
    `Annualised vol: ${(annualisedVol * 100).toFixed(2)}%`,
    sma20 !== null
